...
Queue.prototype.add = function(callback){
	if(this._stack.length === 0){
		...
		console.log(this._callCount+" < "+this.config.maxCalls);
		/*console.log(now+" - "+this._firstCall+" > "+this.config.interval);*/
		...
		}else if((now - this._lastCall) > this.config.interval){
			console.log("reset");
			this._callCount = 2;
			this._firstCall = now;
			setTimeout(callback, 0, null, (this.config.maxCalls - this._callCount), this.config.maxStack);
...
Queue.prototype._pushStack = function(callback){
	if(this._stack.length < this.config.maxStack){
		...
		var timeout = ((this._lastCall + (this.config.interval * factor)) - Date.now()) + (this._stack.length % this.config.maxCalls) + 1;
			console.log("pushstack - "+timeout);	
		this._stack.push({
			callback: callback,
